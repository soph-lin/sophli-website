...
const pc = (
  ...
  opts: Partial<Omit<PullCordHandle, "id" | "icon" | "message" | "category">> = {}
): PullCordHandle => {
  if (!opts.x) opts.x = -10;
...
export const specialHandles: Record<string, PullCordHandle> = {
  ...
  "12/25": pc("christmas", "🎁", "a gift for you on a special day 🎄", "seasonal"),
};

export const miscHandles: PullCordHandle[] = [
  pc("spider", "🕷️", "aaah! spider!", "household", { includeOriginalHandle: true }),
